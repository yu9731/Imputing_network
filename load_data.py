...
variable_name = ['temp', 'solarradiation', 'AT_SFH']
pred_hour = 24
windows_length = 24
...
variable_name = ['temp', 'solarradiation', 'L17']
pred_hour = 24
windows_length = 24
...
pd_test_data = heat_data_test.loc['2023-12-01 00:00:00':'2023-12-31 23:00:00', variable_name]

pd_time = pd_time_information(pd.to_datetime(pd_train_data.index))
pd_time = filtering_by_hours(pd_train_data.values[:,-1], pd_time, pd_time.index)    # numpy from pd_train_data instead of pd_train_data

np_hour = pd_time.loc[:, 'hour'].values
heat_load_train = pd_time.loc[:, 'Energy [KW]'].values

pd_time_test = pd_time_information(pd.to_datetime(pd_test_data.index))
pd_time_test = filtering_by_hours(pd_test_data.values[:,-1], pd_time_test, pd_time_test.index)

np_hour_test = pd_time_test.loc[:, 'hour'].values
heat_load_test = pd_time_test.loc[:, 'Energy [KW]'].values

train_data = np.hstack((pd_train_data.values[:,:2], heat_load_train.reshape((heat_load_train.shape[0],1)), np_hour.reshape((np_hour.shape[0],1))))
test_data = np.hstack((pd_test_data.values[:,:2], heat_load_test.reshape((heat_load_test.shape[0],1)), np_hour_test.reshape((np_hour_test.shape[0],1))))
...
for j in range(train_data.shape[1]):
    #min_data = np.min(train_data[:, j])
    #max_data = np.max(train_data[:, j])
